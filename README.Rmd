...
We have included an addin so you can mark text as a reviewer comment. This will automatically insert code around that selection of text so that it will appear as a numbered comment in papaja::revision_letter_pdf documents. Comments will also be automatically numbered. A hotkey can be set for this addin (e.g. Ctrl+Shift+R).

![animation of ReviewerComment addin](man/figures/README/ReviewerComment.gif)

If a YAML header is not detected in document the addin will add it, Other syntax will also be appended around existing text such that the user can quickly work with revise functions.
